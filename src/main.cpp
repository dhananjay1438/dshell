...
std::vector<std::string> split_line_with_delimiter(std::string str,
                                                   char split) {

  std::vector<std::string> tokens;
  std::string::size_type whitespace, current = 0;

  while ((whitespace = str.find(split, current)) != std::string::npos) {
...
int dshell_execute(std::vector<std::string> args) {
  ...
  if (pid == 0) {
    ...
    if ((strcmp(argv[0], "history")) == 0) {
      ...
      std::cerr << "dshell: command not found: " << argv[0] << "\n";
      return -1;
